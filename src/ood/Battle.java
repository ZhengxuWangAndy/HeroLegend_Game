    ...
    public void useItem(Hero hero){
        ...
        if (serial != 0){
            ...
            if(item.getCategory().equals("equip")){
                ...
                // use other
                //spell choose enemy
                color.purpleOut("Using spell! Who's your target?");
                Monster monster = (Monster) monsterTeam.getRole_prompt();
                if (hero.getMp() < 100){
    ...
    public void start(){
        ...
        while (!(herosTeam.countLeft() == 0 || monsterTeam.countLeft() == 0)) {
            ...
            for (int i = 0; i < herosTeam.roleMap.size(); i++) {
                color.blueOut("Here's your team.");
                ...
                Hero hero = (Hero) herosTeam.getRole_prompt();
            ...
            if (monsterTeam.countLeft() == 0){
                System.out.println(color.redBG("!!! NICE ALL MONSTERS DIED !!!"));
                break;
            }
            //monsters turn
            ...
            for(int i = 1; i <= monsterTeam.roleMap.size(); i++){
                ...
                if (monster.getHp() <= 0){
                    ...
                    int serial = rand.nextInt(1, herosTeam.roleMap.size() + 1);
                    while (herosTeam.getRole(serial).getHp() <= 0){
                        serial = rand.nextInt(1, herosTeam.roleMap.size() + 1);
                    }
                    Hero hero = (Hero) herosTeam.getRole(serial);
                    this.monsterAttack(monster,hero);
                    if (herosTeam.countLeft() == 0){
                        System.out.println(color.redBG(" ALL HEROES DIED "));
